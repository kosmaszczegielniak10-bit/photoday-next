...
const IcHeart = ({ filled }) => filled
    ? <svg viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" width="22" height="22"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
...
export default function FeedPage() {
    ...
    const handleComment = async (postId, text) => {
        ...
        const tempComment = {
            id: tempId,
            text,
            author_name: user?.display_name || user?.username || 'Ja',
            author_avatar: user?.avatar_path,
            created_at: new Date().toISOString()
        };

        setPosts(prev => prev.map(p => p.id === postId
            ? { ...p, comments: [...(p.comments || []), tempComment], comment_count: (p.comment_count || 0) + 1 }
            : p));

        try {
            const c = await api.post(`/feed/posts/${postId}/comments`, { text });
            setPosts(prev => prev.map(p => p.id === postId
                ? { ...p, comments: p.comments.map(cc => cc.id === tempId ? c : cc) }
                : p));
        } catch (err) {
            showToast(err.message, 'error');
            setPosts(prev => prev.map(p => p.id === postId
                ? { ...p, comments: p.comments.filter(cc => cc.id !== tempId), comment_count: Math.max(0, (p.comment_count || 1) - 1) }
                : p));
        }
    };
